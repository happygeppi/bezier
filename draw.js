...
class DJ {
  ...
  cancel = () => (this.speed > 0 ? this.speed-- : undefined);
  addUpdater = () => this.speed++;
  doIf(_cdts, _func) {
    ...
    if (typeof _cdts == "object") {
      ...
      _cdts.forEach((_cdt) => {
        if ((_cdt == CLICK && this.mouse.down) || this.keyPressed(_cdt))
          _func();
      });
  ...
  handleInput() {
    ...
    if (this.mouse.down && typeof mouseDown == "function")
      mouseDown(this.mouse.pos.x, this.mouse.pos.y);
    if (this.keysDown.length > 0 && typeof keyDown == "function")
      keyDown(this.keysDown);
    this.keysPressed = [];
    this.keysReleased = [];
  }
  checkPerformance() {
  ...
  addEventListeners() {
    ...
    document.addEventListener("keyup", (e) => {
      ...
      if (typeof keyUp == "function") keyUp(e.key);
    });
    document.addEventListener("mouseup", (e) => {
      if (typeof mouseUp == "function") mouseUp(e.clientX, e.clientY);
    });
    document.addEventListener("click", (e) => {
      if (typeof mouseClick == "function") mouseClick(e.clientX, e.clientY);
    });
  }
  protoFuncs() {
